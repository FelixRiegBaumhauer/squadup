...
def retUsers():
    ...
    return c.fetchall()[::-1] # get in reverse time order
...
#return all friends of users from users table
def retAllFriends(q):
    db=sqlite3.connect(f)
    c=db.cursor()
    fList = utils.search.retFriends(q)[0] #list of friend names
    retList = []
...
#returns current location of user specified
...
#returns user's location in format of [address, lat, lon]
def maptesting(user, t):
    ...
    for f in friends:
        if t==0:
            ...
            if len(currLoc) > 4: # if not in stuy get inputted location
                ...
                place = utils.schedule.getPeriod(f[1]) # if in stuy, get class room location
            ...
            else: # if list is not long enough for geoguessr game get random places
